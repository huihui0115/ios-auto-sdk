...
本轮新增（Round 62）：**bootstrap Xcode 独立编译热修**——`tools/regenerate-bootstrap.mjs` 现在固定为生成的 `AutoBootstrapScript.m` 导入 `AutoBootstrapScript.h`，解决 Swift Package/Xcode 将源文件作为独立翻译单元编译时 `NSString` 未声明的问题；verify 同步校验导入，未直接手改生成文件，bootstrap JS 内容与 60526/61440 预算不变；
