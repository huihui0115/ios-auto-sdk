...
Round 78 · SDK v1.41.1 · 2026-09-21
...
先连接 VS Code 0.15.0，点击“检查手机状态”。SDK 1.41.0 的 `runtimeHealth`
提供低内存档、解码预算、热状态、省电模式、前后台、运行/停止状态和有限后台额度状态。
上次停止按原因码显示：内存压力、严重发热、后台到期、超时、用户停止、失败或完成。
只记录本进程最后一次结束，不持久化脚本、原始错误、设备标识或凭证；重启/被系统杀死不猜测原因。
检查只读、不自动轮询，不会将 capabilities 声明当作真机权限验收。
内置节点采集逐次返回访问/深度/结果预算和受限原因；老适配器的完整性明确显示未知。
