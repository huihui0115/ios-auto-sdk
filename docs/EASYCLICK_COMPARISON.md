...
| Touch | coordinate click, double click, long press, pressure, multi-touch, swipe, drag | selector activation, coordinate activation, double activation, scroll-view swipe | UIKit adapter cannot inject real touches, long press, pressure, multi-touch, or cross-app gestures; a WDA/private adapter is required |
...
| Input/app control | input-method APIs, helper APIs, Home/app lifecycle and process operations | host text replacement plus WDA app launch/activate/terminate/state helpers and home-screen/lock/unlock endpoints | No system input method, app inventory, or UIKit-adapter cross-app lifecycle control |
| Device | screen/model/OS/battery, app list, serial, orientation, charging | public device/app/screen/battery/orientation information, clipboard/brightness/volume/vibration, and WDA home-screen/lock/unlock | No app inventory, serial number, reboot, install/uninstall, or process control |
| Media | save images/videos to the camera roll through the agent | add-only Photos writes for sandbox images, base64 images, videos and screenshots (`media.*`), gated by `allowMediaLibrary` and an iOS authorization prompt; requires `NSPhotoLibraryAddUsageDescription` | No album-object API, batch import, photo picker, or camera/QR capture |
