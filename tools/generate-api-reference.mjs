...
main();` });
APIS.push({ cat:'touch', sig:'node.children() / node.allChildren() / node.parent() / node.siblings() / node.nextSiblings() / node.previousSiblings()', title:'节点关系遍历（EasyClick 风格）', desc:'EasyClick node 关系方法：在节点对象上直接调用，children() 取直接子节点、allChildren() 递归取所有子孙节点、parent() 取父节点、siblings()/nextSiblings()/previousSiblings() 取兄弟，返回值均为包装好的节点对象，可继续 .click()/.attr() 等操作。', params:[], returns:'AutoNodeObject[] | AutoNodeObject | null', example:`function main(){
  const node = findElement({text: "当前项"});
  const kids = node.children();
  logd("子节点数: " + kids.length);
  logd("子孙节点数: " + node.allChildren().length);
