...
const guides = [
  {
    ...
    body: `
      ...
      <div class="callout warning"><strong>先检查手机状态</strong>连接后点侧栏“检查手机状态”，无需输入代码。能力信息是适配器声明，不是实机验收；跨 App 控制、截图或节点不可用时，不要用死循环重试。</div>
      <h2>接下来做什么</h2>
      <div class="card-grid">
  ...
  },
  {
    ...
    body: `
      ...
      <h2>手机状态：不用代码也能排查</h2>
      <p>连接验证完成会显示状态摘要；点“检查手机状态”获取新快照，可取消。展开详情查看 iOS/SDK 版本、低内存保护、发热、省电模式、前后台、后台额度和上次运行的停止原因；“复制诊断摘要”不包含配对码、设备标识、地址或脚本内容。不会点击手机、执行脚本或持续轮询；断线、切换工作区/设备或运行状态变化会清除旧结果。</p>
      <p>旧版手机端仍能连接，但可能显示“未提供”。此时更新 App 后重试。能力标注“声明支持，未实机验证”；上次原因仅保存在本次 App 进程，系统杀进程或重启后的原因不会凭空推断。</p>
      <h2>常用按钮</h2>
      <table>
        ...
        <tbody>
          ...
          <tr><td>检查手机状态</td><td>只读体检、保护状态、上次停止原因；不需要输入脚本。</td></tr>
          <tr><td>搜索函数 / 插入操作</td><td>搜“小白点、VPN、剪贴板”等中文，选择后插入代码，不会自动执行。输入函数括号查看参数，悬停查看说明。</td></tr>
...
main();`)}
      ...
      <p>侧栏“检查手机状态”可确认是否启用低内存保护，并查看内存压力、严重发热、后台到期、超时或用户停止等退出原因。状态是检查时快照，不是持续监控，也不是永久保活承诺。</p>
  ...
  },
  {
    ...
    body: `
      ...
      <div class="steps">
        ...
        <div class="step"><strong>采集关联快照</strong><p>截图和节点树属于同一轮请求，减少错配；采集可取消。两者仍是先后采集，页面移动时请重采，不是系统级原子快照。</p></div>
      ...
      <h2>稳定选择器原则</h2>
      <p>状态栏区分“节点遍历完成（可访问树）”“部分节点”和“完整性未知”。达到访问、深度或结果数量预算会说明原因，即使返回节点远少于请求数也不会声称完整；旧 App 或未实现预算信息的适配器显示未知。部分/未知结果里没找到，不代表目标不存在。“遍历完成”也不等于所有可见控件都能被辅助功能访问。导出的快照保留预算信息。</p>
